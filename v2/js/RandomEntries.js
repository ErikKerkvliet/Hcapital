...
function store(type, ids) {
	...
	if (type === 'back') {
		...
		let next = localStorage.getItem('random-next');
	}
}

function setState() {
	let back = localStorage.getItem('random-back') ? localStorage.getItem('random-back') : '';
	let next = localStorage.getItem('random-next') ? localStorage.getItem('random-next') : '';
	...
	if (next && state !== STATE_DEFAULT) {
		$('#navigator-next').css('opacity', '1');
	} else {
		$('#navigator-next').css('opacity', '0.2');
	}
}
...
$(document).ready(function () {
	...
	$('#navigator-refresh').click(function () {
		let selected = [];
		$.each($('#type-selector > input'), function() {
			if ($(this).is(':checked')) {
