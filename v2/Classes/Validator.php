...
use v2\Database\Entity\Link; // Added for type hinting
use v2\Resolvers\HostResolver;
use v2\Database\Entity\Host;
use v2\RapidgatorClient;
use v2\ClientException;

// --- Helper functions (if not part of a framework like Laravel) ---
// It's better if these are properly managed by your application's framework or autoloader.
if (!function_exists('request')) {
    // Basic $_REQUEST helper. Adapt if your 'request' function is different.
...
class Validator
{
    ...
    {
        if (empty($this->rapidgatorUrls)) return;

		$client = new RapidgatorClient(getenv('RAPIDGATOR_USERNAME'), getenv('RAPIDGATOR_PASSWORD'), null);
        $urlsToProcess = $this->rapidgatorUrls;
        $processedUrls = [];

		foreach ($urlsToProcess as $url => $originalValue) {
			preg_match('/\/file\/([a-f0-9]{32})\//', $url, $matches);
			$fileId = $matches[1] ?? null;

            if (!$fileId) {
                $processedUrls[$url] = 'error_invalid_url';
                continue;
            }
			...
			try {
                ...
                if (!isset($response->status)) {
                    $processedUrls[$url] = 'error_client_response_format';
                    continue;
                }

                $isAvailable = ($response->status === 200);
                $stateFilter = request('state'); // '1' for available, '2' for unavailable

                if (($stateFilter === '1' && !$isAvailable) || ($stateFilter === '2' && $isAvailable)) {
                    // Filtered out: keep original url => url mapping
                ...
                error_log("Validator: Rapidgator ClientException for URL $url: " . $e->getMessage());
				$processedUrls[$url] = 'unavailable'; // Or 'error_api_exception'
			}
		}
        $this->rapidgatorUrls = $processedUrls;
	}
     ...
     * Modifies $this->mexashareUrls
     */
    private function validateMexashare(): void
    {
        if (empty($this->mexashareUrls)) return;
        
        $originalUrls = $this->mexashareUrls;
        $processedUrls = [];

        foreach ($originalUrls as $url => $originalValue) {
            // Placeholder: Actual Mexashare API call would go here.
            // For now, apply filter logic as if it was always available.
            $isAvailable = true; // Placeholder: assume available
            $stateFilter = request('state');

            if (($stateFilter === '1' && !$isAvailable) || ($stateFilter === '2' && $isAvailable)) {
                 $processedUrls[$url] = $url; // Filtered out, keep url => url
                 continue;
            }
            $processedUrls[$url] = $isAvailable ? 'available' : 'unavailable';
        }
        $this->mexashareUrls = $processedUrls;
    }
}